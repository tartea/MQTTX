  ...
  <div class="script-view rightbar">
    ...
    <div class="script-view-header">
      <div>
        ...
        <el-select size="mini" :value="scripts.some((obj) => obj.id === currentScriptId) ? currentScriptId : null"
          placeholder="" @change="handleScriptChange">
      ...
      <div>
        ...
        <el-tooltip placement="top" :disabled="!inUseScript" :effect="theme !== 'light' ? 'light' : 'dark'"
          :content="$t('script.inUseScript')">
          <span>
            <el-button v-if="currentScriptId" class="delete-btn" :disabled="inUseScript" type="danger" size="mini" plain
              @click="handleDelete">
              {{ $t('common.delete') }}
            </el-button>
          </span>
    ...
    <div class="editor-container script-editor" :style="{
      height: '320px',
    }">
      <Editor v-if="activeTab === functionTab" ref="functionEditor" id="function-editor" :key="1"
        :lang="currentFunction" v-model="functionEditorValue" lineNumbers="on" :lineNumbersMinChars="5"
        renderHighlight="line" @qucik-save="handleTest" />
      <Editor v-if="activeTab === schemaTab" ref="schemaEditor" id="schema-editor" :key="2" :lang="currentFunction"
        :isCustomerLang="true" v-model="schemaEditorValue" lineNumbers="on" :lineNumbersMinChars="5"
        renderHighlight="line" @qucik-save="handleTest" />
    </div>
    <el-row class="script-test-row script-test-input" :gutter="20">
      ...
      <el-col :span="6">
        <el-input v-if="activeTab == schemaTab" :placeholder="$t('script.protoName')" v-model.trim="protoName"
          size="mini" :label="$t('script.protoName')"></el-input>
      </el-col>
      <el-col :span="activeTab == schemaTab ? 3 : 6">
        <el-button class="test-btn" type="outline" size="mini" @click="handleTest">测试{{ $t('script.test') }}</el-button>
      </el-col>
    </el-row>
    <div class="editor-container script-editor script-test-input" :style="{
      height: '80px',
    }">
      <Editor v-if="activeTab === functionTab" :key="3" ref="functionInput" id="function-input"
        :lang="functionEditorLang" lineNumbers="on" :lineNumbersMinChars="2" v-model="functionInputValue" />
      <Editor v-if="activeTab === schemaTab" :key="4" ref="schemaInput" id="schema-input" :lang="schemaEditorLang"
        lineNumbers="on" :lineNumbersMinChars="2" v-model="schemaInputValue" />
    </div>
    <div class="lang-type">
    ...
    <my-dialog :title="$t('script.saveScript')" :visible.sync="showSaveDialog" class="save-script" width="400px"
      @confirm="save" @keyupEnter="save">
      <el-form ref="form" label-position="left" label-width="120px" :model="record">
        <el-row :gutter="20">
          <el-col :span="24">
            <el-form-item :label="activeTab === functionTab ? $t('script.functionName') : $t('script.schemaName')"
              prop="name">
    ...
    <ImportScript @finish="handleUpload"
      :title="activeTab === functionTab ? $t('script.importFunction') : $t('script.importSchema')"
      :extension="extension" :format="activeTab === functionTab ? functionList : schemaList"
