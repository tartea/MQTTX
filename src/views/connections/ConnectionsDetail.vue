<template>
  <div class="connections-detail">
    ...
    <div ref="connectionTopbar" class="connections-topbar right-topbar" :style="{
      left: leftValue,
    }">
      <div class="connections-info">
        <div class="topbar">
          <div class="connection-head">
            <el-tooltip placement="bottom" :effect="theme !== 'light' ? 'light' : 'dark'" :open-delay="500"
              :content="$t('connections.showConnections')">
              <a v-if="!showConnectionList" href="javascript:;" class="show-connections-button" @click="
            ...
            <el-tooltip :disabled="!showTitleTooltip" :effect="theme !== 'light' ? 'light' : 'dark'"
              :content="titleName" :open-delay="500" placement="top">
              <h2 ref="title" :class="[{ offline: !client.connected }, 'title-name']">
                {{ titleName }}
              </h2>
            </el-tooltip>
            <a href="javascript:;" :class="['collapse-btn', showClientInfo ? 'top' : 'bottom']"
              @click="handleCollapse($route.params.id)">
              <i class="iconfont icon-collapse"></i>
            </a>
            <transition name="el-fade-in">
              <el-popover v-if="client.connected" :title="$t('connections.messageCount')" popper-class="message-popover"
          ...
          <div class="connection-tail">
            <transition name="el-fade-in">
              <template v-if="!showClientInfo && client.connected">
                <el-tooltip v-if="sendTimeId" placement="bottom" :effect="theme !== 'light' ? 'light' : 'dark'"
                  :open-delay="500" :content="$t('connections.clearIntervalBtn')">
                  <a class="stop-interval-btn" href="javascript:;" @click="stopTimedSend">
                    <i class="iconfont icon-stop-timing"></i>
                  </a>
                </el-tooltip>
                <el-tooltip v-else placement="bottom" :effect="theme !== 'light' ? 'light' : 'dark'" :open-delay="500"
              ...
              <el-tooltip v-if="!showClientInfo && !client.connected" placement="bottom"
                :effect="theme !== 'light' ? 'light' : 'dark'" :open-delay="500"
            ...
            <el-tooltip v-if="scriptOption !== null" placement="bottom" :effect="theme !== 'light' ? 'light' : 'dark'"
              :open-delay="500" :content="$t('script.removeScript')">
              <a class="remove-script-btn" href="javascript:;" @click="removeScript">
                <i class="iconfont icon-stop-script"></i>
              </a>
            </el-tooltip>
            <el-tooltip v-if="enableCopilot" placement="bottom" :effect="theme !== 'light' ? 'light' : 'dark'"
              :open-delay="500" content="MQTTX Copilot">
            ...
            <template v-if="!isNewWindow">
              <el-tooltip placement="bottom" :effect="theme !== 'light' ? 'light' : 'dark'" :open-delay="500"
                ...
                <a :class="['edit-btn', { disabled: client.connected || connectLoading }]" href="javascript:;"
        ...
        <el-collapse-transition>
          <ConnectionInfo v-show="showClientInfo" class="connection-info" :connection="record" :titleName="titleName"
            :client="client" :btn-loading="connectLoading" @handleConnect="connect" @handleDisconnect="disconnect"
            @handleCancel="cancel" />
        </el-collapse-transition>
      </div>
      <transition name="el-zoom-in-top">
        <div v-show="searchVisible" class="connections-search topbar">
          <el-input id="searchTopic" v-model="searchParams.topic" size="small"
            :placeholder="$t('connections.inputTopic')" @keyup.enter.native="searchContent"
            @keyup.esc.native="handleSearchClose">
          </el-input>
          <el-input v-model="searchParams.payload" size="small" :placeholder="$t('connections.inputMsgContent')"
            @keyup.enter.native="searchContent" @keyup.esc.native="handleSearchClose" class="content-search">
          </el-input>
    ...
    <div class="connections-detail-main" :style="{
      ...
      <div class="connections-body">
        ...
        <SubscriptionsList v-if="$route.params.id" ref="subList" :connectionId="$route.params.id" :record="record"
          :top="bodyTopValue" @onClickTopic="handleTopicClick" @deleteTopic="handleTopicDelete"
          @onSubError="handleSubTopicError" />
        <MessageList ref="msgList" :key="$route.params.id" :subscriptions="record.subscriptions"
          :messages="recordMsgs.list" :height="messageListHeight" :marginTop="messageListMarginTop"
          @showContextMenu="handleContextMenu" @loadMoreMsg="loadMoreMessages" @hideNewMsgsTip="hideNewMsgsTip" />
      ...
      <div ref="connectionFooter" class="connections-footer" :style="{ marginLeft: detailLeftValue }">
        ...
        <MsgPublish :mqtt5PropsEnable="record.mqttVersion === '5.0'" ref="msgPublish" :editor-height="inputHeight - 75"
          :style="{ height: `${inputHeight}px` }" :disabled="sendTimeId !== null" :clientConnected="client.connected"
          :sendTimeId="sendTimeId" @foucs="handleMessages" @handleSend="sendMessage"
          @handleSendTimedMessage="handleCommand('timedMessage')" @onInsertedCode="handleInsertedCode" />
      </div>
    </div>
    ...
    <BytesStatistics ref="bytesStatistics" :visible.sync="showBytes" v-bind="chartData" :version="version"
      :uptime="uptime" />
  </div>
</template>
...
export default class ConnectionsDetail extends Vue {
  ...
  private async handleDeleteMessage() {
    ...
    if (res) {
      ...
      this.$emit('reload')
      this.$log.info(
        `Successfully deleted message from ${this.record.name} with ClientID ${this.record.clientId
    ...
    try {
      ...
      if (id === this.curConnectionId) {
        ...
        if (isFromActiveTopic || isFromNotActiveTopic) {
          let receivedLog = `Message arrived for ${this.record.name} with topic: "${topic}". Message ID: "${message.id
